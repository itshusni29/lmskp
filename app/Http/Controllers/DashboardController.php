    ...
    public function dashboard()
{
    ...
    // Data for Total Classes every day
    $totalClassesData = [];
    $totalClassesDates = [];
    $classes = TrainingClass::all();

    // Loop through the classes to get counts by date
    foreach ($classes as $class) {
        $date = $class->created_at->format('Y-m-d');
        if (!isset($totalClassesData[$date])) {
            $totalClassesData[$date] = 0;
        }
        $totalClassesData[$date]++;
    }
    return view('dashboard', [
        ...
        'totalClassesDates' => json_encode(array_keys($totalClassesData)), // Pass totalClassesDates
        'totalClassesData' => json_encode(array_values($totalClassesData)), // Pass totalClassesData
    ]);
}
