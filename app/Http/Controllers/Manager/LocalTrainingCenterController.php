...
class LocalTrainingCenterController extends Controller
{
    ...
    {
        // Ambil data pengguna yang sedang login (manager)
        $manager = auth()->user();
        ...
        // Filter pengguna berdasarkan department yang sama dengan manager yang sedang login
        // dan berdasarkan date_of_join yang dipilih atau gunakan default
        $selectedDateOfJoin = $request->get('date_of_join', $defaultDoj);
        $users = User::where('date_of_join', $selectedDateOfJoin)
            ->where('department', $manager->department) // Filter berdasarkan department manager
            ->get();
    
        // Jika tidak ada user, langsung return view tanpa post test data
        if ($users->isEmpty()) {
            return view('pages.manager.training_stages.local_training_center', [
                'users' => $users,
